...
class App extends Component {
  ...
  handleLogin = async loginData => {
    ...
    if (this.state.currentUser.username) {
      const getlocations = async () => {
        const locations = await indexLocation(
    ...
    if (updatedLocation) {
      ...
      this.setState(prevState => ({
        locations: prevState.locations.map(location =>
          location.id === parseInt(updatedLocation.id)
            ? updatedLocation
            : location
        )
      }));
  ...
  handleAddActivity = async (locationId, postdata) => {
    ...
    this.props.history.push(
      `/user/${this.state.currentUser.id}/location/${locationId}/activity`
    );
  };
  ...
  getActivities = async locationId => {
    const activities = await indexActivity(
      this.state.currentUser.id,
      locationId
    );
  ...
  render() {
    return (
      <div className="App">
        ...
        <Route
          exact
          path="/user/:id/location/:id/activity"
          render={props => (
            <ActivityList
              currentUser={this.state.currentUser}
        ...
        <Route
          exact
          path="/user/:id/location/:id/activity/add"
          render={props => (
            <ActivityAddForm
              currentUser={this.state.currentUser}
              locationId={props}
        ...
        <Route
          exact
          path="/user/:id/location/:id/activity/:id/edit"
          render={props => (
            <ActivityEdit
              activities={this.state.activities}
